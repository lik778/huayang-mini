...
Page({
  ...
  onLoad: async function (options) {
    ...
    this.setData({
      parentBootCampId: options.parentBootCampId || 0,
      lessonDate
    })
    ...
    // 2022.4.11-JJ
    bxPoint("university_ course_start_page", {
  ...
  async show() {
    // 2022.4.11-JJ
    bxPoint("university_ course_finish_share_click", {
      course_id: this.data.courseInfo.id
    }, false)
  ...
  modifyRestTimeTest(time) {
    this.setData({
      restTimeText: this.calcRemainingTime(time)
    })
  },
  /**
   * 计算剩余时间
  ...
  async switchPrevAction() {
    if (this.data.currentActionIndex > 0) {
      ...
      if (!this.data.isRunning) {
        this.setData({
          isPlayMainPointAudioPlaying: false
        })
        this.toggleAction("play")
        this.data.backgroundMusicAudio.play()
      }
  ...
  toggleAction(status) {
    if (status === "pause") {
      ...
      // 结束全局计时器
      this.setData({
        didPauseRecordGlobalTime: false
      })
  ...
  async playCommand(commands) {
    ...
    this.setData({
      targetActionIndex: this.data.targetActionIndex + 1
    })
  ...
  async prepareNextAction() {
    ...
    if (this.data.currentActionIndex < this.data.actionData.length) {
      ...
      // 2022.4.11-JJ
      bxPoint("university_ course_finish_page", {
        course_id: this.data.courseInfo.id
      })
