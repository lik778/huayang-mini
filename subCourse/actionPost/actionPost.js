...
Page({
  ...
  async cookData({
    ...
    let qrCode = await queryPunchCardQrCode({
      kecheng_id: keChengId,
      traincamp_id: bootCampId == 0 ? '' : bootCampId
    })
  ...
  onShareAppMessage: function () {
    // 2022.4.11-JJ
    bxPoint("course_clock", {
      type: 1,
      course_id: this.data.postData.course_id
    }, false)
    return {
      title: "跟着花样一起变美，变自信",
  ...
  saveToLocal() {
    ...
    bxPoint("course_clock", {
      type: 2,
      course_id: this.data.postData.keChengId
    }, false)
    ...
    this._saveCanvasImageToLocal('actionOrder').then(({
      tempFilePath
    }) => {
