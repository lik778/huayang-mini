		...
		wx.request({
			...
			complete: (res) => {
				if (res.statusCode >= 200 && res.statusCode < 300) {
					...
					if (getCurrentPages()[0].route !== "pages/preserve/preserve") {
						wx.redirectTo({url: "/pages/preserve/preserve"})
					}
					reject(res)
				} else {
					reject(res)
