...
import { queryOrderDetail } from "../../api/course/index"
...
Page({
	...
	onLoad: function (options) {
		...
		// 获取订单信息
		if (orderId) {
			queryOrderDetail({order_id: orderId}).then(({data: res}) => {
				let btnName = ""
				let overtime = false
				...
				this.setData({btnName, overtime, payData, orderId})
				this.residueTime()
			})
		}
	},
	...
	intervalMethods(targetDate, timer) {
		...
		if (gapDate >= 0) {
			...
			this.setData({payData})
		} else {
			clearInterval(timer)
			this.setData({overtime: false, btnName: "支付超时"})
		}
	},
